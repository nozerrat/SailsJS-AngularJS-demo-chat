...
.directive('sistemaGerente', [ function( ) { return {
	restrict: 'E',
	scope: { props:'=' },
	controller:['$scope','resourceService',
		function( $scope , resourceService ) {
			...
			$scope.rsc.urlDel  = '/sistema/delete'; // url de la ruta a enlazar
			$scope.rsc.urlSave = '/sistema/save'; // url de la ruta a enlazar
			$scope.rsc.registerForm( 'sistema' );
			$scope.rsc.ngTable( '/sistema/consult' );
			...
			$scope.rsc.formGroup = function( values ) {
				...
				$scope.propsVersion = { modelName:'version', label:'Versión', type:'text', ariaRules:'required', defaultValue:values, listener: $scope.rsc.listener};
			...
			+ '<div class="table-responsive">'
				+ '<table ng-table="rsc.tableParams" class="table table-bordered table-striped table-hover table-condensed">'
					+ '<tr ng-repeat="row in $data track by $index">'
						...
						+ '<td data-title="\'Versión\'" filter="{version: \'text\'}" sortable="\'version\'">{{row.version}}</td>'
